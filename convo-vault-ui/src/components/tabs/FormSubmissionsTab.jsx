...
export default function FormSubmissionsTab() {
  ...
  useEffect(() => {
    ...
    const fetchForms = async () => {
      ...
      try {
        const response = await billingAPI.getForms(location.id);
        if (response.success) {
  ...
  const buildExportFilters = () => {
    const exportFilters = {};
    if (filters.formId) exportFilters.formId = filters.formId;
  ...
  const handleGetEstimate = async () => {
    ...
    try {
      ...
      const response = await billingAPI.getEstimate(location.id, 'formSubmissions', exportFilters);
      if (response.success) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Export form submissions from this sub-account</p>
        </div>
          ...
          <Tooltip
            title={
              <div style={{ fontSize: '13px', lineHeight: '1.6' }}>
                ...
                <br />
                $0.002 per submission. Volume discounts apply.
              </div>
            }
            placement="left"
          >
      ...
      <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <Select
              value={filters.formId || undefined}
              onChange={(value) => setFilters(prev => ({ ...prev, formId: value || '' }))}
      ...
      <div className="bg-gradient-to-br from-teal-50 to-cyan-50 border border-teal-200 rounded-xl p-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">How Form Submissions Export Works</h3>
            <ul className="space-y-2 text-sm text-gray-700">
              <li className="flex items-start gap-2">
                <span className="text-teal-500 mt-0.5">1.</span>
                <span>We fetch all form submissions in your sub-account matching your filters</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-teal-500 mt-0.5">2.</span>
                <span>Each submission is exported with form ID, contact details, and submitted field values</span>
              </li>
              <li className="flex items-start gap-2">
        ...
        <div className="mt-4 bg-white/60 rounded-lg p-4 border border-teal-100">
          <h4 className="text-sm font-semibold text-gray-800 mb-2">Pricing</h4>
          <div className="flex items-center gap-6 text-sm text-gray-700">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 bg-green-500 rounded-full"></span>
              <span><strong>$0.002</strong> per submission (0.2 cents)</span>
            </div>
            <div className="flex items-center gap-2">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <div className="flex flex-wrap gap-2">
          {['SubmissionID', 'FormID', 'ContactID', 'Name', 'Email', 'CreatedAt', 'OtherFields'].map((col) => (
