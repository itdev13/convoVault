...
/**
 * Internal Messages — exports every INTERNAL CHAT thread in the sub-account (conversationType=6).
 *
 * Internal chat (team-to-team messages) is never returned by the normal message export. This tab
 * discovers every internal thread, gathers each thread's messages, and exports them as one CSV/JSON.
 * Same per-message pricing as Activity Messages ($0.018/message, no volume discount).
 */
export default function InternalMessagesTab() {
  ...
  const handleGetEstimate = async () => {
    ...
    try {
      const response = await billingAPI.getEstimate(location.id, 'internalMessages', buildExportFilters());
      if (response.success) {
  ...
  const handlePayAndExport = async (notificationEmail, format = 'csv') => {
    ...
    try {
      ...
      const response = await billingAPI.chargeAndExport(location.id, 'internalMessages', format, filters, notificationEmail);
      if (response.success) {
        setActiveJob({
  ...
  return (
    <div className="space-y-6">
      <ExportEstimateModal
        ...
        estimatingMessage="Finding internal threads and gathering messages…"
        estimate={estimate}
        error={estimateError}
        exportType="internalMessages"
      />

      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Internal Messages</h2>
          <p className="text-sm text-gray-500 mt-1">
            Export every internal (team-to-team) chat thread in this sub-account. Internal chat is not
            included in the normal Messages export — this tab pulls it in.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
          <p className="text-sm text-amber-800">
            <strong>How it works:</strong> we find every internal chat conversation in this sub-account and
            gather all of their messages. Pricing: <strong>$0.018 / message</strong> — no volume discount.
          </p>
        </div>
      </div>
    </div>
