...
export default function InfoModal({ isOpen, onClose, title, message, details }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 rounded-2xl max-w-lg w-full overflow-hidden animate-fade-in">
        {/* Header */}
        <div className="bg-gradient-to-r from-green-500 to-green-600 px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="text-white hover:bg-slate-700 rounded-full p-1 transition-colors"
            >
            ...
            <div className="space-y-3">
              {details.map((detail, index) => (
                <div key={index} className="bg-slate-700/50 rounded-lg p-4 border border-slate-600">
                  {detail.icon && (
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <div className="font-semibold text-white mb-1">{detail.title}</div>
                        <div className="text-sm text-slate-400">{detail.description}</div>
                  ...
                  {!detail.icon && (
                    <div>
                      {detail.title && <div className="font-semibold text-white mb-1">{detail.title}</div>}
                      {detail.description && <div className="text-sm text-slate-400">{detail.description}</div>}
                    </div>
                  )}
                </div>
              ))}
