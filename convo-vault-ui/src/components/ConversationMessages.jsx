...
export default function ConversationMessages({ conversation, onBack }) {
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-6 text-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="w-14 h-14 bg-slate-700/50 backdrop-blur rounded-full flex items-center justify-center text-2xl font-bold border-2 border-slate-600">
      ...
      {/* Error */}
      {error && (
        <div className="bg-red-900/20 border-1 border-solid border-red-800 rounded-xl p-6">
      ...
      {!isLoading && !error && messages.length > 0 && (
        <div className="flex items-center justify-between bg-slate-800 border-1 border-solid border-slate-600 rounded-xl p-4">
          <div className="flex items-center gap-3">
      ...
      {/* Messages */}
      {!isLoading && !error && messages.length > 0 && (
        <div className="bg-slate-800 rounded-xl border-1 border-solid border-slate-600 p-6 min-h-[400px] space-y-4">
          {messages.map((message) => {
            ...
            return (
              <div key={message.id} className={`flex ${isOutbound ? 'justify-end' : 'justify-start'}`}>
                ...
                >
                  <div className={`text-xs mb-1 flex items-center gap-1.5 ${isOutbound ? 'text-blue-300' : 'text-slate-400'}`}>
                    ...
                    message.meta.email.messageIds.length > 1 && (
                      <div className={`text-xs mb-2 px-2 py-1 rounded flex items-center gap-1.5 ${
                        isOutbound ? 'bg-blue-500/20 text-blue-300' : 'bg-blue-900/20 text-blue-400'
                      }`}>
                        ...
                        <Tooltip
                          ...
                          placement="top"
                        >
                          <svg className={`w-3.5 h-3.5 cursor-help ${isOutbound ? 'text-blue-300' : 'text-blue-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {message.attachments && message.attachments.length > 0 && (
                    <div className="mt-2 flex items-center gap-2 text-xs">
                      <svg className={`w-3.5 h-3.5 ${isOutbound ? 'text-blue-300' : 'text-slate-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                      </svg>
                      <span className={isOutbound ? 'text-blue-300' : 'text-slate-400'}>
                      ...
                      <Tooltip
                        ...
                        placement="top"
                      >
                        <svg className={`w-3.5 h-3.5 cursor-help ${isOutbound ? 'text-blue-300' : 'text-slate-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                      </Tooltip>
                    </div>
      ...
      {/* No Messages State */}
      {!isLoading && !error && messages.length === 0 && (
        <div className="bg-slate-800 rounded-xl border-1 border-solid border-slate-600 p-12 min-h-[400px] flex items-center justify-center">
          <div className="text-center">
            <div className="text-6xl mb-4">💬</div>
            <h3 className="text-xl font-semibold text-white mb-2">No Messages Found</h3>
            <p className="text-slate-400 mb-6">This conversation doesn't have any messages yet</p>
            <button
              onClick={onBack}
      ...
      {/* Footer Stats */}
      {messages.length > 0 && (
        <div className="bg-slate-700/50 border-1 border-solid border-slate-600 rounded-xl p-4">
          <div className="text-sm text-slate-400">
            ...
            {hasMore && <span className="ml-4 text-blue-400">• More available</span>}
          </div>
        </div>
      )}
      </div>
